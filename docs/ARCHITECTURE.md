...
api/
  ...
  alembic/env.py              Migration runtime: registers models, reads DATABASE_URL
  alembic/versions/           Migration scripts, oldest first
  requirements.txt
  Dockerfile
frontend/
...
`database.py` reads `DATABASE_URL` from the environment (no default — it fails fast at import time if unset, rather than silently falling back to something like SQLite that would mask a misconfigured environment). Schema is managed by Alembic (`api/alembic/`), not by an app-startup `create_all()` — see [§3.4.1](#341-migrations) below.

#### 3.4.1 Migrations

Schema changes go through Alembic migrations in `api/alembic/versions/`, not `SQLModel.metadata.create_all()`. `api/alembic/env.py` imports `models` so every table registers on `SQLModel.metadata`, then reads `DATABASE_URL` from the environment the same way `database.py` does — one source of truth for where the database is, not two.

`docker-compose.yml`'s `api` service now runs `alembic upgrade head` before starting `uvicorn`, so a fresh `docker compose up --build` gets a fully migrated schema with no manual step. In any other deployment, run `alembic upgrade head` as a pre-start/release step before the app boots.
...
Always read the autogenerated file before committing it — `--autogenerate` diffs the models against the live DB and is good at catching new/dropped columns and tables, but it doesn't reliably detect things like column renames (it'll see those as a drop + an add, losing data unless you edit the migration by hand).
...
`main.py` adds `CORSMiddleware` restricted to an explicit allowlist read from `CORS_ORIGINS` (comma-separated, defaults to `http://localhost:5173`, the Vite dev server's default origin) rather than `allow_origins=["*"]` — the API accepts cookies/credentials on every request, and a wildcard origin can't be combined with credentials per the CORS spec anyway. Deploying the frontend elsewhere means adding its origin to `CORS_ORIGINS`, not editing code.
...
**Cookie security flags are env-driven, not hardcoded**: `https_only` and `same_site` come from `SESSION_COOKIE_SECURE` / `SESSION_COOKIE_SAMESITE` env vars (default `false` / `lax`, matching local HTTP dev — see `.env.example`), not fixed values. This matters once the app is deployed: `SameSite=Lax` only lets a cookie ride along on cross-*site* requests (not cross-origin ones) when the frontend and API share a registrable top-level domain (e.g. `app.example.com` + `api.example.com`); `localhost:5173`/`:8000` happen to count as same-site too, which is why this was invisible in local dev. If frontend and API end up on unrelated domains instead, the cookie needs `SameSite=None`, which browsers only honor when the cookie is also `Secure` (HTTPS-only) — `main.py` fails fast at startup if `SESSION_COOKIE_SAMESITE=none` is set without `SESSION_COOKIE_SECURE=true`, since that combination is always a misconfiguration, never a valid deployment.
...
**Sync robustness — bounded pages, incremental commits, and Plaid error handling**: the original `sync-transactions` loop called Plaid's `/transactions/sync` in a `while has_more` loop with a single `session.commit()` after the loop ended, and no handling for Plaid API errors. Three problems followed directly from that shape:

1. **Unbounded request duration** — a multi-year backfill runs entirely inline on one request/worker thread with no upper limit, risking client/proxy timeouts.
2. **All progress lost on any failure** — because the cursor was only persisted after every page had been fetched, a transient failure on (say) page 8 of 10 discarded pages 1–7 too; the next attempt re-fetched everything from scratch.
3. **Plaid API errors surfaced as raw 500s** — `ITEM_LOGIN_REQUIRED` (the bank forced a re-auth) and transient errors (`RATE_LIMIT_EXCEEDED`, `PLANNED_MAINTENANCE`) were indistinguishable from a genuine bug, so the frontend had nothing better to show the user than a generic error string.

The fix (`api/routers/plaid.py`):
- **`item.cursor` is saved with a `session.commit()` after every page**, not once at the end. Since `/transactions/sync` is already cursor-based and idempotent ([§3.8](#38-plaid-integration) above), committing per page is safe and means a failure partway through only loses the one page in flight, not everything fetched so far.
- **`MAX_SYNC_PAGES_PER_REQUEST = 20`** caps how many pages one HTTP request will fetch (roughly 10k transactions, since Plaid pages up to ~500 at a time). If more remain, the response's new `has_more` field tells the caller to call sync again — which resumes from the just-committed cursor rather than restarting. This bounds request duration without needing a background job runner, which this stack still doesn't have ([§6](#6-known-limitations--next-steps)).
- **`client.transactions_sync()` calls are wrapped in `try/except ApiException`**, checking the Plaid `error_code` in the exception body: `ITEM_LOGIN_REQUIRED` returns `409` with a message telling the user to reconnect via Plaid Link (re-auth via Link's "update mode" isn't built yet — that's the natural next step this unblocks); known-transient codes (`RATE_LIMIT_EXCEEDED`, `PLANNED_MAINTENANCE`, `PRODUCT_NOT_READY`, `INTERNAL_SERVER_ERROR`) return `503` ("try again shortly"); anything else returns a generic `502`. Every failure is also logged via `logger.warning(...)` with the user/item id so a stuck sync is diagnosable from server logs instead of only visible as a user complaint.

**Frontend implication**: both places that call `syncTransactions()` — `SyncButton` and the post-Link sync in `PlaidLinkButton` — now loop on the response's `has_more` until it's `false`, so a large backfill still completes from a single click; it's just multiple bounded HTTP requests under the hood instead of one unbounded one.
...
- `api` — runs `alembic upgrade head` then FastAPI via `uvicorn --reload`, source bind-mounted so code edits apply without a rebuild.
...
### Applying a schema change locally

Change a model, then generate and apply a migration ([§3.4.1](#341-migrations)):
...
Review the generated file in `api/alembic/versions/`, then either restart the stack (`docker compose up` re-runs `alembic upgrade head` on every start) or apply it directly:

```bash
docker compose exec api alembic upgrade head
```

If a local Postgres volume already has tables from before migrations existed (created by the old `create_all()`), stamp it instead of re-running the initial migration's `CREATE TABLE`s against tables that already exist:

```bash
docker compose exec api alembic stamp 0001_initial_schema
```
...
- **No Plaid Link "update mode" for expired connections** ([§3.8](#38-plaid-integration)) — when Plaid returns `ITEM_LOGIN_REQUIRED` (the bank forced a re-auth), `sync-transactions` now reports this clearly (`409`, "reconnect your account") instead of a raw `500`, but there's no UI flow yet that actually re-opens Plaid Link in update mode to fix it. The only current workaround is running Plaid Link again from scratch, which `exchange-public-token` treats as a fresh connection (new `item_id`/`access_token`, `cursor` reset to `None`) — so the next sync re-walks the account's full transaction history instead of resuming, rather than the lighter re-auth-only flow update mode is meant to provide.
